...
def compute_metrics(samples, qrel_object, query_vectors, kernel_range, \
	...
	for index_key, sample_list in samples.items():
		...
		consistancy_scores.append({'doc_id':doc_id, 'query_id': query_id,\
								   'ktau': consistancy(ranked_lists,stype='kendall'),\
								   'wktau': consistancy(ranked_lists,stype='weighted'),\
								   'pktau': consistancy(pos_ranked_lists,stype='kendall'),\
								   'nktau': consistancy(neg_ranked_lists,stype='kendall'),\
...
def main():
	...
	sample_path='/Users/manishav/workspace/irexplain/samples/pointwise/'
	output_path = '/Users/manishav/workspace/irexplain/explanations_and_metrics/'

	sample_files={
	#uniform prob = uniform
	#tfidf  = tfidf
	sample_path+'tfidf_new_seed/tfidf_0.5_tfidf.txt':output_path+'/tf_idf_0.5/may24/',\
	sample_path+'tfidf_new_seed/tfidf_0.3_tfidf.txt':output_path+'/tf_idf_0.3/may24/',\
	sample_path+'tfidf_new_seed/tfidf_0.1_tfidf.txt':output_path+'/tf_idf_0.1/may24/',\
	sample_path+'mask/samples_mask.ws.5.0.5.txt':output_path+'/mask_0.5/may24/',\
	sample_path+'mask/samples_mask.ws.5.0.1.txt':output_path+'/mask_0.1/may24/',\
	sample_path+'mask/samples_mask.ws.5.0.3.txt':output_path+'/mask_0.3/may24/',\
	sample_path+'uniform/may26/uniform_hack_tfidf_0.5.txt':output_path+'/uniform_words_0.5/may26/',
	sample_path+'uniform/may26/uniform_hack_tfidf_0.1.txt':output_path+'/uniform_words_0.1/may26/',\
	sample_path+'uniform/may26/uniform_hack_tfidf_0.3.txt':output_path+'/uniform_words_0.3/may26/'}
	...
	for ifile, out_path in sample_files.items():
		print(ifile)
		if not os.path.exists(out_path):
			os.mkdir(out_path)
		#name = ifile[ifile.rfind('_samples')+9:ifile.rfind('.')].replace('/','_')
		fname = out_path+ '/'+ifile[ifile.rfind('samples')+8:ifile.rfind('.')].replace('/','_')
