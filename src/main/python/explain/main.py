...
def divergence_from_truth(rel_vector, non_rel_vector,  explain_vector):
    ...
    neg_ent = entropy(all_vectors['non_rel_vector'].values,all_vectors['explain_vector'].values )
    pos_ent = entropy(all_vectors['rel_vector'].values,all_vectors['explain_vector'].values )
...
def compute_metrics(samples, qrel_object, query_vectors, kernel_range, \
    ...
    for index_key, sample_list in samples.items():
        ...
        if len(sample_list) > 50:
        	idx = np.random.choice(np.arange(len(sample_list)), 50, replace=False)
        else:
...
def main():
	...
	samples_path = '/Users/manishav/workspace/irexplain/samples/200_sized_samples/samples_0.9_tfidf.txt'
	...
	ranker_explanation = lime_ranker.LimeRankerExplainer(\
	...
	kernel_range = np.outer(np.logspace(2,5,num=5-2+1,base=10,dtype='int'),\
	...
	for entry in [10,15,20,25,30]:
		compute_metrics(samples, qrel_object, query_vectors, \
		                                  kernel_range, doc_vectors, \
		                                  ranker_explanation, entry, 'tf_idf_0.9')	
